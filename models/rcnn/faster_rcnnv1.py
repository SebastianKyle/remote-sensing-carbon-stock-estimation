...
from .roi_pooling import MultiScaleRoIAlign
from models.backbones.common import FrozenBatchNorm2d
...
class ROIHead(nn.Module):
    ...
    def compute_loss(self, cls_predicts, box_predicts, proposal_idx, gt_boxes):
        ...
        all_cls_idx = (torch.cat(all_cls_idx)).long()
    ...
    def post_process(self, cls_predicts, box_predicts, valid_size):
        predicts = list()
        for cls_pred, box_pred, wh in zip(cls_predicts, box_predicts, valid_size):
            box = box_pred.clone()
            cls = cls_pred.clone()
...
class FasterRCNN(nn.Module):
    ...
    def forward(self, images, valid_size, targets=None):
        ori_image_shape = images.shape[-2:]
        x, targets = self.transformer(images, targets, batched=True)
        ...
        if predicts is not None:
            for predict in predicts:
                result = dict(boxes=predict[:, :4], scores=predict[:, 4], labels=predict[:, 5])
...
def fasterrcnn_resnet50(pretrained, num_classes, pretrained_backbone=True):
    """
